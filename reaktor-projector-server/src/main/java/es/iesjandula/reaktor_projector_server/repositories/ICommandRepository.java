...
public interface ICommandRepository extends JpaRepository<Command, CommandId>
{
	...
	@Query("""
			SELECT NEW es.iesjandula.reaktor_projector_server.dtos.ActionDto(c.action)
			FROM Command c
			GROUP BY c.action
			""")
	...
	@Query("""
			    SELECT NEW es.iesjandula.reaktor_projector_server.dtos.ProjectorModelDto(c.modelName)
			    FROM Command c
