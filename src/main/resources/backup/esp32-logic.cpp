...
// ---------------------------------------------
// WiFi secure client object for HTTPS.
// ---------------------------------------------
...
// ---------------------------------------------
String sslCertFilePath = "/cert.txt";
String sslCertMetadataFilePath = "/sslMetadata.txt";
bool localCertificateExists = false;
bool sdCertificateExists = false;
String sslCert;

// ---------------------------------------------
// Authentication Token paths and control flags.
// ---------------------------------------------
String authTokenFilePath = "/authToken.txt";
String authTokenMetadataFilePath = "/authTokenMetadata.txt";
bool localAuthTokenExists = false;
bool sdAuthTokenExists = false;
String authToken;
...
void setup() {
  ...
  if (sdCardInitialized) {
    ...
    // -------------------------------
    compareAndCopy(sslCertFilePath, sslCertMetadataFilePath, "SSL Certificate File");
    ...
    // -------------------------------
    // Compare and copy the most recent Authentication Token File
    // -------------------------------
    compareAndCopy(authTokenFilePath, authTokenMetadataFilePath, "Authentication Token File");
  ...
  // -------------------------------
  // Configure JWT Token & SSL Certificat
  // -------------------------------

  // Sets the authetincation token from the file.
  setFileContentToStringVariable(authToken, authTokenFilePath, "Authentication Token");

  // Sets the SSL Certificate from the file.
  setFileContentToStringVariable(sslCert, sslCertFilePath, "SSL Certificate");
...
void loadConfigFromFile(String configFilePath) {
  ...
  while (configFile.available()) {
    ...
    if (line.startsWith("SSID=")) {
      ...
      taskServerAddress = line.substring(7);
    }
  }
  configFile.close();
}
...
void callServer(int& httpResponseCode, String& httpResponseData) {
  ...
  if (taskServerAddress.length() == 0) {
    printInterfaceSentence("ERROR: 'taskServerAddress' is not set. Aborting request.");
    return;
  }

  if (authToken.length() == 0) {
    printInterfaceSentence("ERROR: 'authToken' is not set. Aborting request.");
    return;
  }
  ...
  // WIFI CLIENT SETUP
  // ---------------------------------------------
  WiFiClientSecure client;  // Creates a new client for this request.
  // IMPORTANTE ---------------------------------------------------------------------------------------------------------------------------------------
  // Cambiar la configuración insegura por la asignación del certificado sslCert.
  client.setInsecure();
  // client.setCACert(sslCert); // sslCert es una variable String que a estas alturas debe de contener el certificado.
  ...
  String completeURL = taskServerAddress + "?projectorClassroom=" + projectorClassroom;  // Configures the complete request URL.

  printInterfaceSentence("Request details: ");
  printInterfaceSentences("-- Server Address: ", taskServerAddress);
  printInterfaceSentences("-- Specific endpoint: ", "?projectorClassroom=");
  printInterfaceSentences("-- Projector ID: ", projectorClassroom);
  printInterfaceSeparator();

  //http.begin(client, completeURL);  // Use the generated Wifi Client for the request.
  ...
  if (httpResponseCode > 0) {
    ...
    if (httpResponseData.length() > 0) {
      ...
      printInterfaceSentences("- Server Response:", String(httpResponseCode));
    ...
    printInterfaceSentences("-- Reason: ", http.errorToString(httpResponseCode).c_str());
  }
...
String updateEvent(String eventId, String deviceResponseCode) {
  ...
  if (projectorClassroom.length() == 0) {
    debugln("ERROR: Projector classroom parameter is not set. Aborting request.");
    return "";
  }
  ...
  String completeURL = taskServerAddress + "?eventId=" + eventId + "&rarc=" + encodedResponseCode + "&classroom=" + projectorClassroom;
  debugln(completeURL);
  ...
  if (httpResponseCode > 0) {
    ...
    printInterfaceSentences("ERROR: HTTP request failed. Code: ", String(httpResponseCode));
    printInterfaceSentences("ERROR: Reason: ", http.errorToString(httpResponseCode).c_str());
  }
